#Form https://socket.dev/pypi/package/ms21xx-firmware
#2025/03/29 Fix the "TypeError: to_bytes() missing required argument 'byteorder' (pos 2)"
...
def checksum_as_bytes(data: bytes, mask: int = 0xFFFF):
    return checksum(data, mask).to_bytes(2, byteorder='big')
...
def main():
    ...
    if args.chip == "ms2109":
        ...
        video_len = (len(video_data) + 1).to_bytes(1, byteorder='little')
        audio_data = args.audio.encode()
        audio_len = (len(audio_data) + 1).to_bytes(1, byteorder='little')
